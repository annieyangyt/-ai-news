...
def send_smart_push(analysis_report, sendkey, username, repo_name):
    ...
    core_insight = next((l for l in lines if "🌟" in l or "今日重磅" in l), "AI 行业高频信号更新")
    web_url = f"https://{username}.github.io/{repo_name}/"
    push_content = f"### 💡 今日核心洞察\n{core_insight}\n\n---\n🔗 [点击阅读全量深度内参]({web_url})\n\n✨ *Generated by Gemini 2.5 Flash*"
    requests.post(f"https://sctapi.ftqq.com/{sendkey}.send", data={"title": f"🧠 AI 商业内参 - {datetime.now().strftime('%m/%d')}", "desp": push_content})
