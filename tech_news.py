...
def send_smart_push(analysis_report, sendkey, username, repo_name):
    """提取分析报告中的“1️⃣ 事实速览层”内容并发送到微信推送界面"""
    if not sendkey: return

    # 逻辑升级：从全量报告中提取第1部分的内容
    ...
    for line in lines:
        ...
        if start_collecting:
            ...
            if "---" in line or "###" in line:
                break
            # 记录非空行，保留原有的 Markdown 符号（如点位和 Emoji）
            if line.strip():
                facts_lines.append(line.strip())

    # 组合提取到的“图2内容”，如果没有提取到则显示保底信息
    facts_content = "\n".join(facts_lines) if facts_lines else "AI 行业最新深度信号已更新"

    web_url = f"https://{username}.github.io/{repo_name}/"
    
    # 组装推送正文，保持图1的极简风格，但内容量显著提升
    push_content = f"""
### 💡 今日事实速览
{facts_content}

---
🔗 [点击阅读全量深度研报（二阶效应/建议）]({web_url})
...
✨ *Generated by Gemini 2.5 Flash*
    ...
    try:
        requests.post(url, data={"title": f"🧠 AI 商业内参 - {datetime.now().strftime('%m/%d')}", "desp": push_content}, timeout=10)
        print("✅ 微信【事实速览】版推送成功")
